...
std::string RegisterMachine::ToString(bool effective_only) {
   ...
   for (auto* m : private_memory_)
      oss << m->ToString(id_);
   if (self_modifying_) {
      oss << "self_modifying_state:" << id_
          << std::setprecision(std::numeric_limits<double>::max_digits10);
      for (size_t reg = kSelfModifyingFirstRegister;
           reg <= kSelfModifyingDecoyRegister; ++reg) {
         oss << ':' << SanitizeSelfModifyingRawTendency(
             private_memory_[MemoryEigen::kScalarType_]
                 ->working_memory_[reg](0, 0));
      }
      oss << '\n';
   }
   return oss.str();
...
RegisterMachine::RegisterMachine(
   ...
   if (f < static_cast<int>(outcomeFields.size()) &&
       outcomeFields[f].rfind("SM", 0) == 0) {
      self_modifying_ = outcomeFields[f].size() > 2 &&
                        outcomeFields[f][2] == '1';
      ++f;
   }
   ...
   ConfigureSelfModifyingRegisters(program_params, false);
   SeedSelfModifyingWorkingFromConstants(program_params);
}
...
std::array<double, 5> RegisterMachine::MutationProbabilities(
    const std::unordered_map<std::string, std::any>& params) const {
      ...
      double p_add = std::any_cast<double>(params.at("p_instructions_add"));

...
void SaveRecursiveForecast(TPG &tpg, EvalData &eval) {
   ...
   if (tpg.GetParam<int>("activation_function") == 0) {
      pred = WrapVectorActionSoftmax(eval);
   } else if (tpg.GetParam<int>("activation_function") == 1) {
      pred = WrapVectorActionArgmax(eval);
   } else if (tpg.GetParam<int>("activation_function") == 2) {
      pred = WrapVectorActionSigmoid(eval);
   } else {
...
void PrepareRecursiveForecastObs(TPG &tpg, EvalData &eval, bool prime) {
   ...
   if (prime) { 
      ...
      if (tpg.GetParam<int>("activation_function") == 0) {
         auto v = WrapVectorActionSoftmax(eval);
         eval.obs->Set(v);
      } else if (tpg.GetParam<int>("activation_function") == 1) {
         auto v = WrapVectorActionArgmax(eval);
         eval.obs->Set(v);
      } else if (tpg.GetParam<int>("activation_function") == 2) {
         auto v = WrapVectorActionSigmoid(eval);
         eval.obs->Set(v);
      }
   }
}
...
void EvalRecursiveForecast(TPG &tpg, EvalData &eval) {
   ...
   for (int i = 0; i < task->n_prime_ - 1; i++) {
      ...
      // cout << "PRIMING OUTPUT PREDICTIONS: ";
      // for (auto vv : vec) {
      //    cout << vv << endl;
      // }
      eval.sample++;
   ...
   for (eval.n_prediction = 0;
      ...
      // cout << "PREDICTIONS OUTPUT PREDICTIONS: ";
      // for (auto vv : vec) {
      //    cout << vv << endl;
      // }
   ...
   if (tpg.GetParam<int>("reset_per_episode") == 1)
      tpg.removeHebbianWeights();
   delete eval.obs;
}
// }
...
inline void EvalRecursiveForecastViz(TPG &tpg, EvalData &eval,
   ...
   PrintRecursizeForecast(
       "tpg_" + std::to_string(tpg.seeds_[TPG_SEED]) + "_test_t" +
           std::to_string(task->t_start[tpg.GetParam<int>(
               "checkpoint_in_phase")][eval.episode]) +
           ".csv",
       task->t_start[tpg.GetParam<int>("checkpoint_in_phase")][eval.episode],
       std::stoi(tpg.GetParam<std::string>("n_input")), prime_samples_plot,
       eval.sequence_targ, eval.sequence_pred);
   // std::cout << "Completed PrintRecursiveForecast." << std::endl;
   delete eval.obs;
}

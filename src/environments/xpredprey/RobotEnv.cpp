 ...
 * Functions for initializing the environmental objects and the robots,
...
void freeObstacleSamples(float ***samples, int **configuration)
{
    if (*samples != NULL && *configuration != NULL)
    {
        for (int i = 0; i < (*configuration)[2]; ++i)
            free((*samples)[i]);
        free(*samples);
    }
    free(*configuration);
...
void setRobotEnvironmentAssetDirectory(const std::string& path)
{
    assetDirectory = path;
}

void shutdownRobotEnvironment()
{
    if (rob != NULL)
    {
    ...
    free(envobjs);
    envobjs = NULL;
    freeObstacleSamples(&wall, &wallcf);
    freeObstacleSamples(&cylinder, &cylindercf);
    freeObstacleSamples(&scylinder, &scylindercf);
    freeObstacleSamples(&light, &lightcf);
}
 ...
 */
float **
load_obstacle(const char *filename, int *objectcf)

{
   ...
   const std::filesystem::path assetPath = assetDirectory / filename;
   if ((fp = fopen(assetPath.string().c_str(),"r")) == NULL)
	 {
	    throw std::runtime_error("cannot open xpredprey sensor asset: " +
                                 assetPath.string());
	  }
 ...
 */
void savegenotype(char* filename, double* genotype, const int glen, int)

...
public:
	std::mt19937 rng;
	explicit RandomGeneratorPrivate(unsigned int seed) : rng(seed) {}
};

RandomGenerator::RandomGenerator( int seed ) {
	seedv = seed;
	prive = new RandomGeneratorPrivate(static_cast<unsigned int>(seed));
}
...
void RandomGenerator::setSeed( int seed ) {
	this->seedv = seed;
	prive->rng.seed(static_cast<unsigned int>(seed));
}

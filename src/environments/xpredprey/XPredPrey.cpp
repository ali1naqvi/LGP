 ...
 * Native C++ implementation of the xpredprey environment.

 * It is built directly by the repository's CMake project.
 */
 ...
 * env constructor
 */
XPredPrey::XPredPrey(int max_steps, const std::string& asset_directory)
{
    ...
    steps = max_steps;
    setRobotEnvironmentAssetDirectory(asset_directory);
    ...
    //
    // initRobot is defined in RobotEnv.cpp.
...
void XPredPrey::copyObs(float* observation)
{
	cobservation = observation;
}

void XPredPrey::copyAct(float* action)
{
	caction = action;
}

void XPredPrey::copyDone(int* done)
{
	cdone = done;
}

void XPredPrey::copyDobj(double* objs)
{
	dobjects = objs;
}
...
int XPredPrey::isDone()
{
	return *cdone;
}

bool XPredPrey::isCaptured() const
{
    return captured_;
}

void XPredPrey::close()
{
    if (rng != nullptr)
    {
        delete rng;
        rng = nullptr;
        shutdownRobotEnvironment();
    }
}
 ...
 */
int initGroundGradSensor(struct robot *)
{
    return(5);
}
/*
